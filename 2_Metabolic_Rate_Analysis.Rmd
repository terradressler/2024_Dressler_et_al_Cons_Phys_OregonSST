...
#### 1. Resting (or Routine) Metabolic Rate
...
ggsave("Figure_S3.png", dpi=600, height=4, width=6)
...
#### 3. Aerobic Scope - Absolulte (AAS) and Factorial (FAS)

Lower Deschutes 
```{r}
#Step 1: read in data get means and SEMs
AS_LD <- read_csv("Aerobic_Scopes.csv") %>% 
  filter(population=="Deschutes")
...
ggsave("Figure8D.png", dpi=600)
```

Population Comparisons of Aerobic Scopes
```{r}
#Absolute Aerobic Scope - 19C

AS_19 <- read_csv("Aerobic_Scopes.csv") %>% 
  filter(temp==19)

AS_19_sum <- AS_19 %>% 
  group_by(population) %>% 
  summarise(mean_aas = mean(aas), sem_aas = sd(aas)/sqrt(n()), mean_fas = mean(fas), sem_fas = sd(fas)/sqrt(n()))

ggplot(AS_19, aes(x = population, y = aas, color = population))+
  geom_point()+
 geom_pointrange(data = AS_19_sum, aes(x = population, y = mean_aas, ymin=mean_aas-sem_aas, ymax=mean_aas+sem_aas, color = population), size = 1.5, alpha = 0.8)+
   scale_color_manual(values = c("#EC7014",  "#DD3497","#2171B5", "#238B45"))+
   labs(y = expression('Absolute Aerobic Scope (mg '* kg^-1* L^-1*')'), x = "")+
   theme_classic()+
  scale_y_continuous(limits = c(0,20))+
...
ggsave("Figure7A.png", dpi=600)

qqnorm(AS_19$aas)
shapiro.test(AS_19$aas)
#normal distribution
leveneTest(aas~population, data = AS_19)
#equal variance
aov_aas_19 <- aov(aas~population, data = AS_19) 
summary(aov_aas_19)
TukeyHSD(aov_aas_19) 

#Absolute Aerobic Scope - 22C
AS_22 <- read_csv("Aerobic_Scopes.csv") %>% 
  filter(temp==22)
...
ggplot(AS_22, aes(x = population, y = aas, color = population))+
  geom_point()+
 geom_pointrange(data = AS_22_sum, aes(x = population, y = mean_aas, ymin=mean_aas-sem_aas, ymax=mean_aas+sem_aas, color = population), size = 1.5, alpha = 0.8)+
   scale_color_manual(values = c("#EC7014",  "#DD3497","#2171B5", "#238B45"))+
   labs(y = expression('Absolute Aerobic Scope (mg '* kg^-1* L^-1*')'), x = "")+
   theme_classic()+
  scale_y_continuous(limits = c(0,20))+
...
ggsave("Figure7B.png", dpi=600)

qqnorm(AS_22$aas)
shapiro.test(AS_22$aas)
#normal distribution
leveneTest(aas~population, data = AS_22)
#equal variance
aov_aas_22 <- aov(aas~population, data = AS_22) 
summary(aov_aas_22)
TukeyHSD(aov_aas_22) 

#Factorial Aerobic Scope - 19C
ggplot(AS_19, aes(x = population, y = fas, color = population))+
  geom_point()+
 geom_pointrange(data = AS_19_sum, aes(x = population, y = mean_fas, ymin=mean_fas-sem_fas, ymax=mean_fas+sem_fas, color = population), size = 1.5, alpha = 0.8)+
   scale_color_manual(values = c("#EC7014",  "#DD3497","#2171B5", "#238B45"))+
   labs(y = expression('Absolute Aerobic Scope (mg '* kg^-1* L^-1*')'), x = "")+
   theme_classic()+
 geom_hline(yintercept = 3, linetype = "dashed")+
...
ggsave("Figure7C.png", dpi=600)

qqnorm(AS_19$fas)
shapiro.test(AS_19$fas)
#normal distribution
leveneTest(fas~population, data = AS_19)
#equal variance
aov_fas_19 <- aov(fas~population, data = AS_19) 
summary(aov_fas_19)
TukeyHSD(aov_fas_19)


#Factorial Aerobic Scope - 22C
ggplot(AS_22, aes(x = population, y = fas, color = population))+
  geom_point()+
 geom_pointrange(data = AS_22_sum, aes(x = population, y = mean_fas, ymin=mean_fas-sem_fas, ymax=mean_fas+sem_fas, color = population), size = 1.5, alpha = 0.8)+
   scale_color_manual(values = c("#EC7014",  "#DD3497","#2171B5", "#238B45"))+
   labs(y = expression('Absolute Aerobic Scope (mg '* kg^-1* L^-1*')'), x = "")+
   theme_classic()+
 geom_hline(yintercept = 3, linetype = "dashed")+
...
ggsave("Figure7D.png", dpi=600)

qqnorm(AS_22$fas)
shapiro.test(AS_22$fas)
#normal distribution
leveneTest(fas~population, data = AS_22)
#unequal variance
kruskal.test(fas~population, data = AS_22)
