...
MMR_LD_sum <- MMR_LD %>% 
  ...
  summarise(mean_MMR = mean(mmr_72_74), sem_MMR = sd(mmr_72_74)/sqrt(n()))

#Step 2: plot MMR data

ggplot(MMR_LD, aes(x = as.factor(temperature) , y = mmr_72_74, color = as.factor(temperature))) +
...
qqPlot(MMR_LD$mmr_72_74)

#normal distribution

leveneTest(mmr_72_74~as.factor(temperature), data = MMR_LD)

#equal variance

LD_MMR_aov <- aov(mmr_72_74~as.factor(temperature), data = MMR_LD)
summary(LD_MMR_aov)
TukeyHSD(LD_MMR_aov)
...
ggplot(MMR_JD, aes(x = as.factor(temperature) , y = mmr_72_74, color = as.factor(temperature))) +
  geom_boxplot(width = 0.4)+
  geom_point(size = 2.5, alpha = 0.8)+
  scale_x_discrete(name = expression("Temperature " ( degree*C)))+
...
qqPlot(MMR_JD$mmr_72_74)

#normal distribution

leveneTest(mmr_72_74~as.factor(temperature), data = MMR_JD)

#equal variance

JD_MMR_aov <- aov(mmr_72_74~as.factor(temperature), data = MMR_JD)
summary(JD_MMR_aov)
...
ggplot(MMR_NU, aes(x = as.factor(temperature) , y = mmr_72_74, color = as.factor(temperature))) +
  geom_boxplot(width = 0.4)+
  geom_point(size = 2.5, alpha = 0.8)+
  scale_x_discrete(name = expression("Temperature " ( degree*C)))+
...
qqPlot(MMR_NU$mmr_72_74)

#normal distribution

leveneTest(mmr_72_74~as.factor(temperature), data = MMR_NU)

#equal variance

NU_MMR_aov <- aov(mmr_72_74~as.factor(temperature), data = MMR_NU)
summary(NU_MMR_aov)
...
ggplot(MMR_S, aes(x = as.factor(temperature) , y = mmr_72_74, color = as.factor(temperature))) +
  geom_boxplot(width = 0.4)+
  geom_point(size = 2.5, alpha = 0.8)+
  scale_x_discrete(name = expression("Temperature " ( degree*C)))+
...
qqPlot(MMR_S$mmr_72_74)

#not a normal distribution

leveneTest(mmr_72_74~as.factor(temperature), data = MMR_S)

#equal variance

S_MMR_aov <- aov(mmr_72_74~as.factor(temperature), data = MMR_S)
summary(S_MMR_aov)
...
AS_LD_sum <- AS_LD %>% 
  group_by(Population,Temp) %>% 
  summarise(mean_aas = mean(aas), min_aas = min(aas), sem_aas = sd(aas/sqrt(n())), mean_fas = mean(fas), sem_fas = sd(fas)/sqrt(n()), mean_MMR = mean(mmr_72), sem_MMR = sd(mmr_72)/sqrt(n()), mean_RMR = mean(rmr_72), sem_RMR = sd(rmr_72)/sqrt(n()), n = n(), min_bw=min(BW)*1000, max_bw=max(BW)*1000)
...
ggplot(BH_Scopes_sum)+
  ...
  geom_point(data = BH_Scopes, aes(y = mmr_72, x = Temp, col = as.factor(Temp)),size =2, alpha = 0.6, shape = "triangle")+
  geom_point(data = BH_Scopes, aes(y = rmr_72, x = Temp, col = as.factor(Temp)),size =2, alpha = 0.6)+
  scale_color_manual(values = c("#FEE391", "#EC7014" ,"#993404"))+
